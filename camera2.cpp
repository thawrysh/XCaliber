...
constexpr uint8_t CameraServer2::kMagicNumber[];

CameraServer2* CameraServer2::GetInstance() {
  static CameraServer2 instance;
  return &instance;
}

CameraServer2::CameraServer2()
    : m_camera(),
      m_serverThread(&CameraServer2::Serve, this),
      m_captureThread(),
      m_imageMutex(),
      m_newImageVariable(),
      m_dataPool(3),
...
void CameraServer2::FreeImageData(
...
void CameraServer2::SetImageData(uint8_t* data, unsigned int size,
...
void CameraServer2::SetImage(Image const* image) {
...
void CameraServer2::AutoCapture() {
  ...
  while (true) {
    ...
    {
      std::lock_guard<priority_recursive_mutex> lock(m_imageMutex);
      if (m_camera != m_camera_new) {
    	  m_camera->StopCapture();
    	  m_camera->CloseCamera();
    	 // m_camera->;
    	  m_camera = m_camera_new;
...
void CameraServer2::StartAutomaticCapture(char const* cameraName) {
...
void CameraServer2::StartAutomaticCapture(std::shared_ptr<USBCamera> camera) {
  //modified
  ...
  m_captureThread = std::thread(&CameraServer2::AutoCapture, this);
  m_captureThread.detach();
  m_autoCaptureStarted = true;
}

bool CameraServer2::IsAutoCaptureStarted() {
  std::lock_guard<priority_recursive_mutex> lock(m_imageMutex);
  return m_autoCaptureStarted;
}

void CameraServer2::SetSize(unsigned int size) {
  std::lock_guard<priority_recursive_mutex> lock(m_imageMutex);
  if (!m_camera) return;
...
void CameraServer2::SetQuality(unsigned int quality) {
  std::lock_guard<priority_recursive_mutex> lock(m_imageMutex);
  m_quality = quality > 100 ? 100 : quality;
}

unsigned int CameraServer2::GetQuality() {
  std::lock_guard<priority_recursive_mutex> lock(m_imageMutex);
  return m_quality;
}

void CameraServer2::Serve() {
